...
class CreateMerkleNodes < ActiveRecord::Migration[7.2]
  def change
    ...
    create_table :merkle_nodes do |t|
      t.string :tree_hash, null: false, index: true # 一棵树的唯一标识
      ...
      # 对应 Event 的 nonce，用于排序，时间早的为树的左节点，晚的为右节点, parent 包含最新子节点的 nonce
      # 每棵子树都是连续的一系列节点，只需要记录开始和结束节点 nonce
      # 通过 nonce range 可以:
      # - 查询所有后代
      # - 查询直接父
      # - 查询所有祖先
      t.integer :begin_nonce, null: false, index: true # begin_nonce = 0 的为创世节点 (同 level 没有更老的了)
      t.integer :end_nonce, null: false, index: true
